      ...
      return (
        <div className={`flex w-full md:w-1/3 items-center ${styles}`}>
          ...
          <input
            ...
            type='text'
            className='w-full md:w-64 p-2 outline-none bg-transparent text-base rounded-xl border-2'
            placeholder={placeholder}
          />
    
          <AiOutlineCloseCircle
            className=' md:flex text-gray-600 text-xl cursor-pointer'
            onClick={clearInput}
          />
        </div>
      );
...
const Header = ({
  ...
  return <div  className='bg-[#f7fdfd]' >
      <div className={`container mx-auto px-5 ${
          ...
          <SearchInput
              placeholder='Position Title or Keywords'
           ...
           <div>
              <CustomButton
                ...
                title='Search'
                containerStyles={
                  "text-white py-2 md:py3 px-3 md:px-10 focus:outline-none bg-blue-600 rounded-full md:rounded-md text-sm md:text-base hover:bg-opacity-70  "
                }
              />
            </div>
          </div>
          {type && (
            ...
            <div className='w-1/4 h-full absolute mt-2 top-24 md:-top-6 lg:-top-14 right-16 2xl:right-[18rem]'>
          <img src={demo} className='object-contain' />
        </div>
      </div>
    </div>;
};

// this is a sign up component which is result in a sign form in our Webapp
import React, { Fragment, useState } from "react";
import { Dialog, Transition } from "@headlessui/react"; // direct UI form headless UI website
...
const SignUp = ({ open, setOpen }) => {
  ...
  const [isRegister, setIsRegister] = useState(false); // isRegister State
  const [accountType, setAccountType] = useState("seeker"); //seeker is someone seeking job
  const [errMsg, setErrMsg] = useState(""); // error messsage
  const {
    register,
    handleSubmit,
    getValues,
              ...
              >
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all ">
                  ...
                  <button
                    ...
                    <button
                      className={`flex-1 px-4 ml-1 py-2 rounded text-sm outline-none ${
                        accountType !== "seeker"
                          ? "bg-[#1d4fd862] text-blue-900 font-semibold"
                          : "bg-white border border-blue-400"
                      }`}
                  ...
                  >
                    <TextInput
                      ...
                      label='Email Address'
                      placeholder={`${
                        accountType === "seeker" ? "User Email" : "Company mail"
                      }`}
                      type='email'
